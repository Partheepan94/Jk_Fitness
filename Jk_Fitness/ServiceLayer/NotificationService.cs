    ...
    public class NotificationService
    {
        ...
        public WebResponce ListNotification(string EmpId)
        {
            try
            {
                ...
                notificationVM.PackageExpirationThisMonth = uow.MembershipRepository.GetAll().Where(x => x.PackageExpirationDate.Date.Month == DateTime.Now.Date.Month).ToList();
                notificationVM.PackageExpirationNextMonth = uow.MembershipRepository.GetAll().Where(x => x.PackageExpirationDate.Date.Month == DateTime.Now.Date.AddMonths(1).Month).ToList();
                notificationVM.PackageExpirationLastMonth = uow.MembershipRepository.GetAll().Where(x => x.PackageExpirationDate.Date.Month == DateTime.Now.Date.AddMonths(-1).Month).ToList();

                notificationVM.MemberShipExpirationThisMonth = uow.MembershipRepository.GetAll().Where(x => x.MembershipExpirationDate.Date.Month == DateTime.Now.Date.Month).ToList();
                notificationVM.MemberShipExpirationNextMonth = uow.MembershipRepository.GetAll().Where(x => x.MembershipExpirationDate.Date.Month == DateTime.Now.AddMonths(1).Month).ToList();
                notificationVM.MemberShipExpirationLastMonth = uow.MembershipRepository.GetAll().Where(x => x.MembershipExpirationDate.Date.Month == DateTime.Now.AddMonths(-1).Month).ToList();
